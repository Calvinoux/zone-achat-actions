...
st.set_page_config(page_title="🎯 AlphaZone | Scanner Long Terme", layout="centered", page_icon="📈")

# 🎨 CSS PRO (Centré, Proportions strictes, Overlay TradingView)
st.markdown("""
<style>
    ...
    :root {
        ...
        --green: #00ff00; --red: #ef4444; --yellow: #f59e0b;
    }
    ...
    /* CONTENEUR CENTRÉ STRICT */
    .app-wrapper { max-width: 1000px; margin: 0 auto; padding: 1.5rem 0; }
    
    /* PANNEAU DE CONTRÔLE */
    .control-panel {
        background: var(--bg-card); border: 1px solid var(--border); border-radius: 16px;
        padding: 1.5rem; margin: 1rem 0; box-shadow: 0 6px 24px rgba(0,0,0,0.35);
    }
    ...
    .control-panel input, .control-panel select {
        ...
        color: var(--text-primary) !important; border-radius: 10px !important; padding: 0.55rem 0.8rem !important; width: 100% !important;
    ...
    .control-panel .stButton > button {
        background: var(--accent-gradient) !important; color: white !important; border: none !important; border-radius: 12px !important;
        padding: 0.85rem 0 !important; font-weight: 600 !important; font-size: 1rem !important; width: 100% !important;
        transition: all 0.25s cubic-bezier(0.4,0,0.2,1) !important; box-shadow: 0 4px 14px rgba(59,130,246,0.35) !important;
    }
    .control-panel .stButton > button:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(59,130,246,0.5) !important; }
    .control-panel .stButton > button:active { transform: translateY(0) scale(0.98) !important; }
    ...
    .delta-pos { color: var(--green); font-size: 0.8rem; margin-top: 0.2rem; }
    .delta-neg { color: var(--red); font-size: 0.8rem; margin-top: 0.2rem; }
    
    /* GRAPHIQUE & OVERLAY */
    .chart-box { border-radius: 14px; overflow: hidden; border: 1px solid var(--border); background: var(--bg-card); margin: 1.5rem 0; position: relative; }
...
def tv_chart_html(symbol, interval, z_mid, ath, atl, currency):
    # Mapping Yahoo -> TradingView
    tv_sym = symbol.replace('.PA', ':EURONEXT').replace('.L', ':LSE').replace('.F', 'FRA:')
    tv_sym = tv_sym.replace('.DE', ':XETRA').replace('.CO', ':COPENHAGEN').replace('.AS', ':AMS')
    if ':' not in tv_sym: tv_sym = f"NASDAQ:{tv_sym}"
    tv_int = {'1wk': 'W', '1mo': 'M', '1d': 'D'}.get(interval, 'W')
    
    # Calcul position Y approximative sur le graphique (clamped 8% - 92%)
    y_pct = max(8, min(92, (1 - (z_mid - atl) / (ath - atl)) * 100))
    
    return f"""
    <div style="position:relative; width:100%; height:580px; background:#131620;">
        <div id="tv_{symbol.replace('.','_')}" style="height:100%; width:100%;"></div>
        <!-- OVERLAY ZONE VERTE -->
        <div style="position:absolute; left:0; right:0; top:{y_pct}%; height:3px; background:#00ff00; box-shadow:0 0 12px rgba(0,255,0,0.8); z-index:10;"></div>
        <div style="position:absolute; right:12px; top:calc({y_pct}% - 14px); background:#00ff00; color:#000; padding:3px 10px; border-radius:6px; font-family:Inter,sans-serif; font-size:12px; font-weight:700; z-index:11; box-shadow:0 2px 8px rgba(0,0,0,0.4);">ZONE: {currency}{z_mid:.2f}</div>
        
        <script src="https://s3.tradingview.com/tv.js"></script>
        <script>
        new TradingView.widget({{
            "autosize": true, "symbol": "{tv_sym}", "interval": "{tv_int}", "timezone": "Etc/UTC",
...
st.markdown("<h1 style='text-align:center; font-size:2rem; margin-top:1rem;'>🎯 AlphaZone <span style='color:#64748b; font-weight:400;'>Scanner Long Terme</span></h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align:center; color:#94a3b8; margin:-0.5rem 0 1rem;'>Détection algorithmique de zones d'accumulation institutionnelles</p>", unsafe_allow_html=True)

# 🎛️ PANNEAU DE CONTRÔLE (PROPORTIONS STRICTES, AUCUNE CASE VIDE)
with st.container():
    st.markdown("<div class='control-panel'>", unsafe_allow_html=True)
    col_top1, col_top2 = st.columns([2.5, 1], gap="medium")
    with col_top1: company = st.text_input("🏢 Nom de l'entreprise", value="Apple", key="company")
    with col_top2: 
...
if analyze:
    with st.spinner("⚙️ Calcul des confluences..."):
        ...
        # 📊 GRAPHIQUE TRADINGVIEW + OVERLAY VERT
        st.components.v1.html(tv_chart_html(ticker, interval, z_mid, ath, atl, currency), height=590, scrolling=False)
        ...
        with cx: st.markdown(f"<div class='card'><div class='metric-label'>📈 Rendement Total ({hold_years} ans)</div><div class='metric-value' style='color:var(--green)'>+{total_ret:.1f}%</div></div>", unsafe_allow_html=True)
        with cy: st.markdown(f"<div class='card'><div class='metric-label'>💰 Annualisé</div><div class='metric-value' style='color:var(--green)'>+{annual_ret:.1f}% / an</div></div>", unsafe_allow_html=True)
